...
gulp.task('build:scripts:main', function() {
    return gulp.src([
        // paths.jsMainFiles + paths.jsPattern, // Main JS folder
        // '/node_modules/iro/dist/iro.min.js',
        // './source/_assets/js/libs/iro.js',
        './source/_assets/js/libs/swiper.min.js',
        // './source/_assets/js/libs/tabletop.js',
        './source/_assets/js/main.js',
        './source/_assets/js/form.js',
...
//     return gulp.src([
//         paths.jsLibsFiles + paths.jsPattern
//         ])
//         .pipe(gulp.dest(paths.jekyllJsFiles))
//         .pipe(gulp.dest(paths.siteJsFiles))
//         .on('error', gutil.log);
...
// // Build all scripts.
// gulp.task('build:scripts', ['build:scripts:main', 'build:scripts:solo', 'build:scripts:libs']);
gulp.task('build:scripts', ['build:scripts:main']);
...
// Copy root-items assets to docs/root
gulp.task('root-items', function() {
    return gulp.src('source/_assets/gulp/root-items/*')
        .pipe(gulp.dest('docs/'))
});

gulp.task('build:json', function() {
    return gulp.src(paths.jsonFilesGlob)
        .pipe(gulp.dest(paths.jekyllJsonFiles))
...
gulp.task('build:jekyll:local', function() {
    // var shellCommand = 'bundle exec jekyll build --config _config.yml,_config.test.yml,_config.dev.yml && gulp root-items';
...
gulp.task('clean', ['clean:jekyll', 'clean:fonts', 'clean:images', 'clean:scripts','clean:styles']);
